...
export class PlaneRenderer extends SceneComponent implements IPlaneRenderer {
  ...
  static async addNode(sdk) {
    await registerPlaneRenderer(sdk);

		let node = await sdk.Scene.createNode();
		var xr = node.addComponent("mp.planeRenderer");
    return node;
  }
  ...
  onInit() {
    ...
    this.mesh = new THREE.Mesh(
      new THREE.PlaneBufferGeometry(100.0, .3),
...
export function registerPlaneRenderer(sdk: any) {
